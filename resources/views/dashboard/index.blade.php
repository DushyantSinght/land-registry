...
<div class="row g-3 mb-4">
    <div class="col-sm-6 col-xl-3">
        <div class="stat-card" style="background:linear-gradient(135deg,#1a56db,#3b82f6)">
            ...
            <div class="mt-2" style="font-size:.76rem;opacity:.8;">
                {{ $stats['registered'] }} Registered &bull; {{ $stats['available'] }} Available
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-xl-3">
    ...
    <div class="col-sm-6 col-xl-3">
        <div class="stat-card" style="background:linear-gradient(135deg,#d97706,#f59e0b)">
            ...
            <div class="mt-2" style="font-size:.76rem;opacity:.8;">
                {{ $stats['approved_registrations'] }} Approved Total
            </div>
        </div>
    </div>
    <div class="col-sm-6 col-xl-3">
        <div class="stat-card" style="background:linear-gradient(135deg,#7c3aed,#8b5cf6)">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="stat-value">₹{{ number_format(($stats['total_stamp_duty'] + $stats['total_reg_fee']) / 100000, 1) }}L</div>
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('registrations.index', ['status'=>'pending']) }}" class="btn btn-sm btn-outline-primary">View All</a>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        @forelse($pendingRegistrations as $reg)
                            <tr>
                                <td><span class="badge bg-warning text-dark">{{ $reg->registration_number }}</span></td>
                                <td class="small">{{ $reg->property->survey_number }}</td>
                                <td class="small">{{ Str::limit($reg->owner->full_name, 20) }}</td>
                                <td class="small text-muted">{{ $reg->registration_date->format('d M Y') }}</td>
                                <td>
                                    <a href="{{ route('registrations.show', $reg) }}" class="btn btn-xs btn-outline-primary" style="font-size:.72rem;padding:2px 8px;">View</a>
                                </td>
                            </tr>
                        @empty
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        @forelse($recentTransfers as $t)
                            <tr>
                                <td><small class="text-muted">{{ $t->transfer_number }}</small></td>
                                <td class="small">{{ $t->property->survey_number }}</td>
                                <td class="small">{{ Str::limit($t->fromOwner->full_name, 12) }} → {{ Str::limit($t->toOwner->full_name, 12) }}</td>
...
new Chart(ltCtx, {
    type: 'doughnut',
    data: {
        labels: {!! json_encode($landTypes->keys()->map(fn($k) => ucwords(str_replace('_',' ',$k)))->values()) !!},
        datasets: [{
            data: {!! json_encode($landTypes->values()) !!},
...
new Chart(mrCtx, {
    type: 'bar',
    data: {
        labels: {!! json_encode($monthlyRegs->keys()) !!},
        datasets: [{
            label: 'Registrations',
            data: {!! json_encode($monthlyRegs->values()) !!},
